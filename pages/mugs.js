    ...
    <div>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
         ...
         {Object.keys(products).length===0&& <p>Sorry currently all the mugs are our of stock. New Arrival coming soon! Stay tuned</p>}
  ...
  for(let item of products){
      if(item.title in mugs){
          ...
          if(!mugs[item.title].size.includes(item.size)&&item.availableQty>0){
              mugs[item.title].color.push(item.size)
          }
      }else{
